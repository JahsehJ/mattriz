...
const t = (key: MessageKey, values?: Record<string, string | number>): string => translate(locale, key, values);
...
function renderResultMatrix(): string {
  ...
  const transformedVectors = workspace.vectors.map((vector) =>
    applyMatrixToVector(workspace.dimension, totalTransform, vector.components).slice(0, workspace.dimension)
  );
  const entries = Array.from({ length: workspace.dimension }, (_, componentIndex) =>
    transformedVectors
      .map(
...
function getMatrixDropTarget(event: DragEvent): DropTarget | null {
  ...
  const matrixItems = Array.from(stackElement.querySelectorAll<HTMLElement>(".matrix-item[data-matrix-id]"));
...
function getVectorDropTarget(event: DragEvent): DropTarget | null {
  const vectorLabels = Array.from(stackElement.querySelectorAll<HTMLElement>(".vector-column-label[data-vector-column-id]"));
  return getSortableDropTarget(event, vectorLabels, "vectorColumnId");
}

function getSortableDropTarget(event: DragEvent, items: HTMLElement[], datasetKey: "matrixId" | "vectorColumnId"): DropTarget | null {
...
function getVectorSourceElement(target: HTMLElement): HTMLElement | null {
  ...
  return stackElement.querySelector<HTMLElement>(`.vector-column-label[data-vector-column-id="${cssEscape(vectorId)}"]`);
}
...
function setDraggingElement(element: HTMLElement | null, event: DragEvent): void {
  ...
  if (element.dataset.vectorColumnId) {
    stackElement
      .querySelectorAll<HTMLElement>(`.vector-expression [data-vector-column-id="${cssEscape(element.dataset.vectorColumnId)}"]`)
      .forEach((item) => {
  ...
  if (event.dataTransfer) {
    ...
    event.dataTransfer.setData("text/plain", element.dataset.matrixId ?? element.dataset.vectorColumnId ?? "");
    const preview = createDragPreview(element);
    event.dataTransfer.setDragImage(preview, getDragOffsetX(element, event, preview), getDragOffsetY(element, event, preview));
  }
}

function clearDraggingElement(): void {
  if (draggedElement) delete draggedElement.dataset.dragging;
  stackElement.querySelectorAll<HTMLElement>(".vector-expression [data-dragging]").forEach((item) => {
...
function createDragPreview(element: HTMLElement): HTMLElement {
  draggedPreviewElement?.remove();
  const preview = element.dataset.vectorColumnId ? createVectorDragPreview(element.dataset.vectorColumnId) : cloneDragPreview(element);
  ...
  preview.innerHTML = `
    ...
    <div class="vector-drag-preview-values" style="grid-template-columns:54px">
      ${vector.draftComponents
        ...
        .map((component) => `<output class="vector-drag-preview-cell">${escapeHtml(component)}</output>`)
        .join("")}
    </div>
  `;
  return preview;
  ...
  const composedProgress =
    state.animation.status !== "idle" && state.animation.mode === "composed"
      ? Math.max(
          ...
          Math.min(
            1,
            ((state.animation.status === "paused" ? state.animation.pausedAt : now) - state.animation.startedAt) / Math.max(1, getAnimationDuration(workspace))
          )
        )
      : null;
  stackElement.querySelectorAll<HTMLElement>(".matrix-item[data-matrix-id]").forEach((item) => {
    const isActive = composedProgress !== null ? true : item.dataset.matrixId === activeStep?.matrixId;
