...
export function multiplyMatrix(dimension: Dimension, a: MatrixValues, b: MatrixValues): MatrixValues {
...
export function composeMathNotation(dimension: Dimension, matrices: MatrixValues[]): MatrixValues {
  return matrices.reduce((total, matrix) => multiplyMatrix(dimension, total, matrix), identityMatrix(dimension));
}

export function lerpMatrix(dimension: Dimension, from: MatrixValues, to: MatrixValues, amount: number): MatrixValues {
  const t = clamp(amount, 0, 1);
  const length = dimension === 2 ? 4 : 9;
  return Array.from({ length }, (_, index) => from[index] + (to[index] - from[index]) * t) as MatrixValues;
}

export function applyMatrixToVector(dimension: Dimension, matrix: MatrixValues, vector: VectorValues): Vec3 {
  if (dimension === 2) {
