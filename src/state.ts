...
export function createMatrixNode(dimension: Dimension, label: string, values?: MatrixValues): MatrixNode {
...
export function createVectorNode(dimension: Dimension, label: string, color: string): VectorNode {
...
export function getTotalTransform(workspace: Workspace): MatrixValues {
  return composeMathNotation(workspace.dimension, workspace.matrices.map((matrix) => matrix.values));
}
...
export function getRenderState(state: AppState, now: number): RenderState {
  ...
  return {
    ...
    transform: getAnimatedTransform(workspace, state.animation, now, totalTransform),
    totalTransform,
...
export function getAnimatedTransform(
  ...
  const elapsed = Math.max(0, (animation.status === "paused" ? animation.pausedAt : now) - animation.startedAt);
  if (animation.mode === "composed") {
    const duration = Math.max(1, workspace.matrices.reduce((sum, matrix) => sum + matrix.durationMs, 0));
    return lerpMatrix(workspace.dimension, identityMatrix(workspace.dimension), totalTransform, elapsed / duration);
  }

  return getStepTransform(workspace, elapsed);
}

export function getStepTransform(workspace: Workspace, elapsedMs: number): MatrixValues {
  ...
  for (const matrix of applicationOrder) {
    ...
    if (remaining <= duration) {
      const partial = lerpMatrix(workspace.dimension, identityMatrix(workspace.dimension), matrix.values, remaining / duration);
      return multiplyMatrix(workspace.dimension, partial, accumulated);
    }

    accumulated = multiplyMatrix(workspace.dimension, matrix.values, accumulated);
    remaining -= duration;
  }
...
export function getActiveStepMatrixId(workspace: Workspace, animation: AnimationState, now: number): string | null {
  return getActiveStepProgress(workspace, animation, now)?.matrixId ?? null;
}

export function getActiveStepProgress(workspace: Workspace, animation: AnimationState, now: number): ActiveStepProgress | null {
  if (animation.status === "idle" || animation.mode !== "steps") return null;

  let remaining = Math.max(0, (animation.status === "paused" ? animation.pausedAt : now) - animation.startedAt);
  const applicationOrder = [...workspace.matrices].reverse();
...
export function getAnimationDuration(workspace: Workspace): number {
  return workspace.matrices.reduce((sum, matrix) => sum + Math.max(1, matrix.durationMs), 0);
}
